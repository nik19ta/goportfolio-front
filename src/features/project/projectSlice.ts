...
import fetchSetState, {addTextToProject, deleteCategory,
  editCategory, fetchAddPhoto, fetchCreateNewCategory,
  fetchCreateProject, fetchDeleteProject,
  fetchRenameProject, fetchSetPrewiew} from './projectAPI';

const initialState: ProjectState = {} as ProjectState;


export const createProject = createAsyncThunk(
    'Project/CreateProject', async (categoryId: string) => {
      const response = await fetchCreateProject(categoryId);
      ...
      return {categoryId, uuid: response.data.project};
    });

export const deleteProject = createAsyncThunk(
    'Project/DeleteProject', async (uuid: string) => {
...
export const renameProject = createAsyncThunk(
    'Project/RenameProject', async (data: {uuid: string, title: string}) => {
      await fetchRenameProject(data.uuid, data.title);
      return {uuid: data.uuid, title: data.title};
    });

export const chengePhoto = createAsyncThunk(
    'Project/ChengePhotoProject', async (data: {uuid: string, image: File}) => {
...
export const addPhotoProject = createAsyncThunk(
    'Project/AddPhotoProject', async (data: {uuid: string, image: File}) => {
      const resp = await fetchAddPhoto(data.uuid, data.image);
      return {uuid: data.uuid, photo: resp.data.upload};
    });

export const setStateProject = createAsyncThunk(
    'Project/ChengeState', async (data: {uuid: string, state: number}) => {
      await fetchSetState(data.uuid, data.state);
      return {uuid: data.uuid, state: data.state};
    });

export const createNewCategory = createAsyncThunk(
    'Category/Create', async (title: string) => {
      const resp = await fetchCreateNewCategory(title);
      return {created: resp.data.created, title};
    });

export const changeExistingCategory = createAsyncThunk(
    'Category/Edit', async (data: {uuid: string, title: string}) => {
      await editCategory(data.uuid, data.title);
      return {uuid: data.uuid, title: data.title};
    });

export const deleteExistingCategory = createAsyncThunk(
    'Category/Delete', async (uuid: string) => {
      await deleteCategory(uuid);
      return uuid;
    });

export const AddDescritionToProject = createAsyncThunk(
    'Project/AddDescription', async (data: {uuid: string, text: string}) => {
      const resp = await addTextToProject(data.uuid, data.text);
      return {
        uuid: resp.data.description_id,
        text: data.text,
