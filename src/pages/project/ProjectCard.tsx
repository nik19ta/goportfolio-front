...
const ProjectCard: FC<IProjectCard> = ({uuid, name, prewiew, state, toHandler, editHandler, deleteHandler, setImageHandlerFirstStap, setImageHandler, owner, setState}) => {
  ...
  return <div className={styles.card_link_btn} onClick={(e: any) => {
        ...
        {owner && <button id="edit_btn" className={styles.edit_icon} onClick={() => {editHandler(uuid)}} >
          <EditFilled  />
        </button>}

        {owner && <button id="del_btn" className={styles.delete_icon} onClick={() => {deleteHandler(uuid)}} >
          <DeleteFilled />
        </button>}

        {owner && <button id="img_btn" className={styles.set_img_icon} >
          <label htmlFor="file-input" onClick={() => setImageHandlerFirstStap(uuid)} > <FileImageOutlined /> </label>
          <input id="file-input" type="file" onChange={(e: any) => {
            if (e.target.files.length > 0) setImageHandler(e.target.files[0])
          }} />
        </button>}
